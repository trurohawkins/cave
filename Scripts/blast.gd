...
func getCollisions():
	...
	hit.sort_custom(compareToCenter)
	for h in hit:
		GM.checkCollision(h)
		
func compareToCenter(a, b):
	return a[1].distance_to(global_position/32) < b[1].distance_to(global_position/32)
